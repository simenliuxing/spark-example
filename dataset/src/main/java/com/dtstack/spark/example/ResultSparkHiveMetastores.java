...
public class ResultSparkHiveMetastores {
    public static void main(String[] args) {
        SparkConf sparkConf = new SparkConf().setAppName("hive_demo");
        ...
        String target_table = "create table default.mysql_data select" +
        ...
        spark_session.sql(target_table);
        spark_session.sql("CACHE  TABLE default.mysql_data");
        //把mysql查出来的表名筛选
        Dataset<Row> sql = spark_session.sql("select distinct(table_name) as table_name from default.mysql_data");
        ...
        for (int j = 0; j < table_num.size(); j++) {
            ...
            JavaRDD<Row> rowJavaRDD = spark_session.sql("desc " + target_db + "." + target_tbl).toJavaRDD();
            List<String> collect = rowJavaRDD.filter(new Function<Row, Boolean>() {
                @Override
                public Boolean call(Row row) throws Exception {
                    if (row.getString(0).startsWith("#")) {
                        return false;
                    }
                    return true;
                }
            ...
            //遍历一张表创建一个字段的集合
            Set<String> field_set=new HashSet<String>(collect);
            ArrayList<String> field_list = new ArrayList<String>(field_set);
            int table_total=0;
...
//          遍历一个表所有的列
        ...
        spark_session.sql("insert into default.result_table_information " +
                ...
                " from default.mysql_data as mysql_data " +
        ...
        spark_session.sql("UNCACHE TABLE default.mysql_data");
