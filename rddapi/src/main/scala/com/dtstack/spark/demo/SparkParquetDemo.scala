...
object SparkParquetDemo {
  def main(args: Array[String]): Unit = {
    val sparkSession: SparkSession = SparkSession.builder().master("yarn").appName("Read Parquet").getOrCreate()

    val input: String = args(0)
    val output: String = args(1)

    val dataFrame: DataFrame = sparkSession.read.parquet(input)
    //dataFrame.show()
